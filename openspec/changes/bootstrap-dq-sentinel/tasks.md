...
- [◐] 7.1 Full RESOLVED loop proven end-to-end on LIVE infra 2026-06-04 via the agent-resolvable **stale-data / freshness** scenario (no source mutation needed): loosen_signal (account_holders) freshness 4541min>180 → Gemini diagnosed stale-data → `resync_connection` → approved → real Fivetran resync → freshness re-check 4541→0 → `status=resolved` (TTR total 3m11s / agent 2m34s / sync_wait 37s). Key learning: only a full resync rewrites `_fivetran_synced`; a no-op sync of an unchanged Sheet does not. The PRD §8 scenario-1 schema-drift variant additionally needs a source-revert (rename the Sheet header back) to reach resolved — that's the manual pre-stage step for the recorded demo. UPDATE 2026-06-04: schema-drift now DETECTABLE — `scripts/seed_loan_products_clean_baseline.py` seeds the known-clean baseline; loop proven live to detect + diagnose the rename (Gemini → HIGH, resync_tables, root cause = base_apr→headline_apr, exact MCP preview), gate held on reject (no write). Also: hosted service has `DQ_FRESHNESS_SLA_MINUTES=10` so the freshness demo is repeatable on any connector idle >10min.
...
- [x] 8.3 README updated — hosted URL, approval-gate screenshot + explainer, quickstart (`uv run uvicorn ...` / `./deploy.sh`), corrected stack (Gemini 3.5 Flash / ADK / FastAPI gate), full repo layout, demo links. License badge correct (Apache-2.0 detected + visible on GitHub). Devpost writeup drafted at `docs/demo/DEVPOST.md`. Demo deck/script/video at `docs/demo/`.
